 ...
 *      user's Turnkey wallet from the persisted identity public key, with NO
 ...
 * and fetching its pubkey over the network on every request — is now avoided by
 * persisting the wallet's identity public key (a stable, non-secret, per-wallet
 * value) at user creation. [createTurnkeySigner] with `identity_public_key` set
 * serves the identity key and its Spark address from that value, touching
 * Turnkey for neither. A user with no persisted key yet (a fresh provision, or a
 * row predating this) is built once WITHOUT the key — which materializes the
 * enclave accounts and lazily fetches the pubkey — and the fetched key is then
 * persisted for every later init.
 *
...
class SdkAccess(
    ...
    suspend fun <T> withUserContext(userId: String, op: suspend (BreezSdk, UserContext) -> T): T {
        ...
        // The persisted identity pubkey (turnkey only; null until the user is
        // provisioned, then the signer is rebuilt network-free from it).
        return withSigner(userId, row.turnkeySubOrgId, row.turnkeyWalletId, row.turnkeyIdentityPubkey) { sdk ->
            op(sdk, ctx)
        }
    }
     ...
     * so the signer inputs are passed explicitly. `identityPubKey` is the pubkey
     * [provisionSigner] just returned, so even this first connect is
    ...
    ): T = withSigner(userId, turnkeySubOrgId, turnkeyWalletId, identityPubKey, op)

    /**
     * One-time Turnkey signer provisioning for a freshly created wallet. This is
     * the only call that pays the Turnkey init round trips: building the signer
     * unseeded materializes the enclave identity accounts, and
     * [get_identity_public_key] lazily fetches the identity pubkey. The returned
     * hex pubkey is persisted by the caller and replayed (seeded) on every later
     * init, which then makes no Turnkey call.
     */
    suspend fun provisionSigner(subOrgId: String, walletId: String): String {
        val signers = createTurnkeySigner(turnkeyConfig(subOrgId, walletId, identityPubKey = null))
        return bytesToHex(signers.sparkSigner.getIdentityPublicKey().bytes)
    }
    ...
    private suspend fun <T> withSigner(
        ...
        val builder = when (signerMode) {
            ...
            SignerMode.TURNKEY -> {
                ...
                val signers = turnkeySigners(userId, subOrgId, walletId, identityPubKey)
    ...
    /**
     * The delegated-key Turnkey config for a user's sub-org/wallet. A non-null
     * [identityPubKey] (compressed secp256k1, hex) seeds the signer so init makes
     * no Turnkey round trip for the identity key or Spark address; null fetches
     * them lazily (and materializes the enclave accounts).
     */
    private fun turnkeyConfig(subOrgId: String, walletId: String, identityPubKey: String?): TurnkeyConfig {
        ...
        return TurnkeyConfig(
            ...
            // Null = Turnkey's documented per-suborganization default (10 RPS).
            maxRps = null,
        )
    }

    /**
     * Builds the per-user Turnkey signers. With a persisted identity pubkey this
     * is network-free (the signer serves the identity key and Spark address from
     * it). Self-healing: a user with no pubkey yet (provisioned lazily, or a row
     * predating this column) is built unseeded — which materializes the enclave
     * accounts and lazily fetches the pubkey — and the fetched pubkey is then
     * persisted so every later init is network-free.
     */
    private suspend fun turnkeySigners(
        ...
        if (identityPubKey != null) {
            return createTurnkeySigner(turnkeyConfig(subOrgId, walletId, identityPubKey))
        }
        log.info("turnkey identity pubkey missing (user={}): provisioning once", userId)
        val signers = createTurnkeySigner(turnkeyConfig(subOrgId, walletId, identityPubKey = null))
        persistIdentityPubKey(userId, bytesToHex(signers.sparkSigner.getIdentityPublicKey().bytes))
        return signers
    }

    /** Stores the wallet's identity pubkey (non-secret, plaintext) on the row. */
    private fun persistIdentityPubKey(userId: String, pubkeyHex: String) {
        ds.connection.use { conn ->
            conn.prepareStatement("UPDATE users SET turnkey_identity_pubkey = ? WHERE user_id = ?").use { ps ->
                ps.setString(1, pubkeyHex)
    ...
    private data class SignerRow(
        ...
        /** Wallet identity pubkey (compressed, hex); null until provisioned. */
        val turnkeyIdentityPubkey: String?,
    )

    private fun loadSignerRow(userId: String): SignerRow? {
        ds.connection.use { conn ->
            conn.prepareStatement(
                "SELECT signer, turnkey_sub_org_id, turnkey_wallet_id, turnkey_spark_address, turnkey_identity_pubkey FROM users WHERE user_id = ?"
            ).use { ps ->
